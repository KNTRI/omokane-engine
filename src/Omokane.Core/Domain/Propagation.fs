    ...
    let 検証する (信号: 伝播信号) : Result<unit, string list> =
        ...
        let エラー一覧 =
            [
                ...
                not (有限である 信号.発生位置.X), "伝播信号の発生位置.Xが有限値ではありません。"
                not (有限である 信号.発生位置.Y), "伝播信号の発生位置.Yが有限値ではありません。"
                信号.方向
                |> Option.exists (fun 方向 -> not (有限である 方向.X)),
                "伝播信号の方向.Xが有限値ではありません。"
                信号.方向
                |> Option.exists (fun 方向 -> not (有限である 方向.Y)),
